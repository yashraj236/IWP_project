...
export default function Comments (props) {
  ...
  const deleteComment = comment => event => {
    ...
    return (<div>
        <CardHeader
              avatar={
                <Avatar className={classes.smallAvatar} src={'/api/users/photo/'+auth.isAuthenticated().user._id+'?'+new Date().getTime()}/>
              }
              title={ <TextField
        ...
        { props.comments.map((item, i) => {
            return <CardHeader
                      avatar={
                        <Avatar className={classes.smallAvatar} src={'/api/users/photo/'+item.postedBy._id+'?'+new Date().getTime()}/>
                      }
